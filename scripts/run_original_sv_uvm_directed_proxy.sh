...
jobs="${IBEX_ORIG_SV_UVM_DIRECTED_JOBS:-64}"
configs=(${IBEX_ORIG_SV_UVM_DIRECTED_CONFIGS:-opentitan})
compile_out="${IBEX_ORIG_SV_UVM_DIRECTED_COMPILE_OUT:-generated/uvm-compile-directed-all}"
out_root="${IBEX_ORIG_SV_UVM_DIRECTED_OUT:-generated/original-sv-uvm-directed-proxy}"
build_root="${IBEX_ORIG_SV_UVM_DIRECTED_BUILD_ROOT:-build/original-sv-uvm-directed-proxy}"
term_after_cycles="${IBEX_ORIG_SV_UVM_DIRECTED_TERM_AFTER_CYCLES:-20000000}"
timeout_s="${IBEX_ORIG_SV_UVM_DIRECTED_TIMEOUT_S:-300}"
ram_size_bytes="${IBEX_ORIG_SV_UVM_DIRECTED_RAM_SIZE_BYTES:-4194304}"
test_filter="${IBEX_ORIG_SV_UVM_DIRECTED_TEST_FILTER:-}"
test_exclude="${IBEX_ORIG_SV_UVM_DIRECTED_TEST_EXCLUDE:-}"
pmp_load_mode="${IBEX_ORIG_SV_UVM_DIRECTED_PMP_LOAD_MODE:-original-bin}"
pmp_bin_base_addr="${IBEX_ORIG_SV_UVM_DIRECTED_PMP_BIN_BASE_ADDR:-0x80000000}"
...
for cfg in "${configs[@]}"; do
  ...
  fusesoc --cores-root externals/ibex --cores-root dv/original_sv_proxy \
    ...
    --build \
    "local:ibex_chisel:original_sv_uvm_directed_simple_system:0.1" \
    "${cfg_opts[@]}" \
...
run_one() {
  ...
  local sim_bin="${repo_root}/${build_root}/${cfg}/ibex_simple_system/local_ibex_chisel_original_sv_uvm_directed_simple_system_0.1/sim-verilator/Vibex_simple_system"
    ...
    if [[ ! -f "${vmem}" || "${bin}" -nt "${vmem}" ]]; then
      case "${pmp_load_mode}" in
        ...
        elf-offset)
          ...
          ;;
        *)
          echo "error: unknown IBEX_ORIG_SV_UVM_DIRECTED_PMP_LOAD_MODE=${pmp_load_mode}" >&2
          return 1
          ;;
      esac
    fi
  ...
  if [[ "${IBEX_ORIG_SV_UVM_DIRECTED_KEEP_GOING:-1}" != 1 ]]; then
...
total="$(wc -l < "${cmds}")"
echo "[original-sv-uvm-directed] running ${total} directed binary simulations with ${jobs} jobs"
...
echo "[original-sv-uvm-directed] completed"
